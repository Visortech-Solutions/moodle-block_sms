...
require_once("textit_api.php");

class SMSNotifier {
    private $type;
    private $apilist = array("nexmo", "twilio", "clickatell", "textit");
    private $instance;
    ...
    public function process_sms($users, $text) {
        ...
        foreach ($usersdetail as $detail) {
            if (!empty($detail->phone)) {
                if ($this->type == "textit") {
                    // Format phone to follow textit's format
                    $phonenumber = "tel:" .$detail->phone;
                }
                else {
                    $phonenumber = $detail->phone;
                }
                
                $status = $this->instance->send_sms($phonenumber, $text);
                if ($status === true) {
